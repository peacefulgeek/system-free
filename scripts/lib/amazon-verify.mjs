/**
 * amazon-verify.mjs — HTTP GET-based ASIN verification with soft-404 detection.
 * Used by cron jobs to verify affiliate links are still valid products.
 */
...
export async function verifyAsin(asin) {
  ...
  try {
    ...
    // Check availability
    const availSection = html.match(/id="availability"[\s\S]*?<\/div>/i)?.[0] || '';
    const inStock = /in stock/i.test(availSection) || /Add to Cart/i.test(html);
    const unavailable = /currently unavailable/i.test(availSection);
    const availability = unavailable ? 'unavailable' : inStock ? 'in-stock' : 'unknown';

    return { asin, valid: true, title, price, availability, url };
  } catch (err) {
