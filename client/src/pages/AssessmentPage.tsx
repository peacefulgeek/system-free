...
function generateSections(slug: string): Section[] {
  ...
  // Default sections for assessments without specific banks
...
function getAssessmentResult(score: number, maxScore: number) {
  ...
  if (pct >= 0.8)
    return {
      level: "Strong Position",
      color: "text-health",
      description:
        "Your assessment reveals a strong foundation for healthcare sovereignty. You have the knowledge, financial resilience, and practical infrastructure to operate largely outside the traditional system. Focus on optimizing and helping others.",
      recommendations: [
        "Consider mentoring others on their sovereignty journey",
        "Review your plan annually to adapt to life changes",
        "Explore advanced strategies like medical tourism and concierge medicine",
      ],
    };
  if (pct >= 0.6)
    return {
      ...
      color: "text-liberty",
      description:
        "You are making real progress toward healthcare independence. Key foundations are in place, but there are specific areas that need attention before you can fully step outside the system.",
      recommendations: [
        "Shore up your emergency fund to cover at least one major medical event",
        "Research DPC providers in your area and schedule a consultation",
        "Start tracking your total healthcare costs to identify savings opportunities",
      ],
    };
  if (pct >= 0.4)
    return {
      ...
      description:
        "You are in the early stages of healthcare sovereignty. The awareness is there, but the practical infrastructure needs significant development. This is a normal starting point — most people begin here.",
      recommendations: [
        "Read our Start Here guide for a structured introduction",
        "Calculate your true annual healthcare costs using our calculator",
        "Begin building a health emergency fund, even with small contributions",
      ],
    };
  return {
    level: "Assessment Needed",
    color: "text-destructive",
    description:
      "Your assessment reveals significant dependency on the traditional healthcare system. This is not a judgment — it is a starting point. The fact that you took this assessment means you are already thinking differently.",
    recommendations: [
      "Start with our Start Here page for foundational knowledge",
      "Understand your current insurance policy completely before exploring alternatives",
      "Begin an emergency fund specifically for healthcare costs",
    ],
  };
}
...
export default function AssessmentPage() {
  ...
  const globalIndex = sections
    ...
    .reduce((sum, s) => sum + s.questions.length, 0) + currentQ;
  ...
  const downloadResults = () => {
    const lines = [
      `Free From the System — Assessment Report`,
    ...
    for (const section of sections) {
      ...
      for (const q of section.questions) {
        ...
        lines.push(`  ${q.q}`);
        lines.push(`  Answer: ${answerText} (${answerScore}/4)`);
        qIndex++;
      }
    }
    ...
    a.href = url;
    a.download = `${assessment.slug}-report.txt`;
    a.click();
  ...
  return (
    <Layout>
      <SEOHead
        ...
        url={`/assessment/${assessment.slug}`}
      />
      ...
      <section className="container py-12 md:py-20">
        <div className="max-w-2xl mx-auto">
          {!showResults ? (
            <>
              <p className="text-sm font-semibold uppercase tracking-widest text-health mb-2">
                {assessment.category} &middot; Section {currentSection + 1} of{" "}
                {sections.length}
              </p>
              <p className="text-xs text-muted-foreground mb-4">
                {sections[currentSection].title} &middot; Question{" "}
              ...
              <div className="w-full h-1.5 bg-cream-dark rounded-full mb-8">
                <div
                  ...
                  style={{
                    width: `${((globalIndex + 1) / totalQuestions) * 100}%`,
                  }}
                />
              </div>

              <div className="space-y-3">
                {sections[currentSection].questions[currentQ].options.map(
                  (option, i) => (
                    <button
                      key={i}
                      onClick={() =>
                        handleAnswer(
                          sections[currentSection].questions[currentQ].scores[i]
                        )
                      }
            ...
            <>
              ...
              <div className="bg-card rounded-lg p-8 border border-border mb-8">
                ...
                <h3 className="font-serif text-lg text-liberty mb-3">
                  Recommendations
                </h3>
                <ul className="space-y-2">
                  {result.recommendations.map((rec, i) => (
                    ...
                    >
                      <span className="text-health font-bold mt-0.5">
                        {i + 1}.
                      </span>
                      {rec}
                    </li>
              ...
              <div className="space-y-4 mb-8">
                <h3 className="font-serif text-lg text-liberty">
                  Section Breakdown
                </h3>
                {(() => {
                  let idx = 0;
                  return sections.map((section) => {
                    ...
                    return (
                      <div
                        key={section.title}
                        className="bg-cream-dark rounded-lg p-4"
                      >
                        ...
                        <div className="w-full h-2 bg-white rounded-full">
                          <div
                            className="h-full bg-health rounded-full"
                            style={{
                              width: `${(sectionScore / sectionMax) * 100}%`,
                            }}
                          />
                        </div>
                      </div>
